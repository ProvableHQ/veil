...
| [`@provablehq/veil-aleo-sdk`](./provable-sdk) | Local accounts, signing, and proving via `@provablehq/sdk`. Build a client from a private key. |
| [`@provablehq/veil-aleo-wallet-adapter`](./wallet-adapter) | Bridge any Provable-standard wallet (Shield, Leo, Puzzle, Fox) into a Veil client. |
| [`@provablehq/veil-aleo-react-hooks`](./react) | `VeilProvider` + `useVeilWallet()` — wallet connection and clients for React apps. |
...
| [`@provablehq/veil-aleo-devnode`](./devnode) | Run and drive a local Aleo devnode for tests. |
| [`@provablehq/veil-leo`](./leo) | Typed wrapper around the `leo` CLI (build, deploy, …). |
| [`@provablehq/veil-aleo-bridges`](./bridge) | Cross-chain bridge client (preview). |

## How they fit together

- **Read-only or server-side?** `@provablehq/veil-core` + `@provablehq/veil-aleo-sdk`.
- **Browser dApp with a wallet?** `@provablehq/veil-core` + `@provablehq/veil-aleo-wallet-adapter` (or `@provablehq/veil-aleo-react-hooks`).
- **Trading the DEX?** `@provablehq/shield-swap-sdk` on top of either signer path.
- **Typed contract bindings?** `@provablehq/veil-codegen`.
- **Local testing?** `@provablehq/veil-aleo-devnode` (+ `@provablehq/veil-leo` to compile/deploy).
