...
A viem-like TypeScript interface for trading on the Aleo AMM, `shield_swap_v0_0_1.aleo`.
...
- Program: `shield_swap_v0_0_1.aleo` (Aleo testnet)
...
**ABI source for the build:** unlike loyalty (which builds Leo locally to get `build/abi.json`),
shield_swap is consumed as a **deployed** program. So the build needs an **ABI-extraction step:
deployed `.aleo` → `abi.json`** (via core `parseProgram`, or `leo build` of the fetched amm-v3
source). Decision: commit a **pinned `abi.json` snapshot** (the deployed ABI is public) + a regen
script that re-fetches and re-extracts; the `loading-dex-context` skill is the *dev* freshness
path, the snapshot is the reproducible *build* artifact.
