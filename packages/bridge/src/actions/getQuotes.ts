...
/**
 * Parameters for `getQuotes`. Chain identifiers are case-sensitive (`ALEO`,
 * `SOLANA`, `EVM:1`); asset codes are chain-qualified (`ALEO_MAINNET`), never
 * bare symbols; `amountIn` is a decimal string in display units with at most
 * the asset's decimals.
 *
 * @property srcChain Source chain identifier.
 * @property destChain Destination chain identifier.
 * @property srcAsset Chain-qualified source asset code.
 * @property destAsset Chain-qualified destination asset code.
