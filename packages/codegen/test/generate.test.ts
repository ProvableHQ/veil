...
describe('generate', () => {
  describe('header', () => {
    it('includes program name and imports', () => {
      ...
      expect(output).toContain("import type { RecordValue, FutureValue, PublicClient, WalletClient, ABI } from '@veil/core'")
  ...
  describe('contract factory', () => {
    ...
    it('generates typed interface with named params and typed returns', () => {
      ...
      expect(output).toContain('mint: (params: { recipient: string, amount: bigint } & { fee?: bigint }) => Promise<{ transactionId: string, result: [Token, FutureValue] }>')
    })
    ...
    it('generates correct typed interface for loyalty token', () => {
      ...
      expect(output).toContain('mint_card: (params: { recipient: string, initial_points: bigint, nonce: string }) => Promise<[LoyaltyCard, FutureValue]>')
      // execute: named params + fee, returns typed result
      expect(output).toContain('mint_card: (params: { recipient: string, initial_points: bigint, nonce: string } & { fee?: bigint }) => Promise<{ transactionId: string, result: [LoyaltyCard, FutureValue] }>')
      // read
