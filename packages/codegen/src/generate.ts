...
export function generate(options: GenerateOptions): string {
  ...
  lines.push('')
  lines.push(`import { getContract, parseAbi } from '${coreImport}'`)
  lines.push(`import type { RecordValue, PublicClient, WalletClient, ContractInstance } from '${coreImport}'`)
  lines.push('')
...
function generateAbiConstant(abi: ABI): string[] {
  // Serialize the ABI as a JSON constant so the factory doesn't need a separate file read
  return [
    `/** The parsed ABI for ${abi.program}. */`,
    `export const ABI_JSON = ${JSON.stringify(abiToRaw(abi), null, 2)} as const`,
  ]
}

/** Convert parsed ABI back to the raw JSON format parseAbi expects */
function abiToRaw(abi: ABI): Record<string, unknown> {
  return {
    program: abi.program,
    structs: abi.structs,
    records: abi.records,
    mappings: abi.mappings,
    storage_variables: abi.storageVariables,
...
function generateContractFactory(abi: ABI): string[] {
  ...
  // e.g. "loyalty_token.aleo" → "LoyaltyToken"
  ...
  lines.push(` *`)
  lines.push(` * @param publicClient - Public client for reading mappings`)
  lines.push(` * @param walletClient - Wallet client for executing functions`)
  lines.push(` * @returns A contract instance with validated function and mapping names`)
  lines.push(` */`)
  lines.push(`export function create${factoryName}Contract(`)
  lines.push(`  publicClient?: PublicClient,`)
  lines.push(`  walletClient?: WalletClient,`)
  lines.push(`): ContractInstance {`)
  lines.push(`  const abi = parseAbi(ABI_JSON)`)
  lines.push(`  const client = publicClient && walletClient`)
  lines.push(`    ? { public: publicClient, wallet: walletClient }`)
  lines.push(`    : publicClient ?? walletClient!`)
  lines.push(`  return getContract({ program: PROGRAM_ID, abi, client })`)
  lines.push(`}`)
