...
| `fills` | A position's swap fills: how each pool swap, in chain order, changed the tokens backing its fixed liquidity. Replays recent ones, or streams them live with `--watch`. | Session |
...
7. **`shield-swap positions`** — watch what the position holds and earns.
   `shield-swap fills --position … --watch` shows the other side of that: each
   swap in the pool, as it lands, and what it did to the position's inventory.
