...
/**
 * Live matrix over the Provable gateway's auth paths: unauthenticated (the
 * default, since the gateway needs no credentials) and the provisioned-key
 * model (one `X-API-Key` header per request). Each configured path runs the
 * same two probes — a record scan and a delegated proof with on-chain
 * confirmation — so a regression in either path, or a drift between them,
 * fails the same file.
 *
 ...
 *
 * Gated behind VEIL_INTEGRATION=1. The api-key path also needs
 * EDGE_PROVABLE_API_KEY. The account defaults to the SDK demo account (funded
 * on testnet, public balance); override with VEIL_E2E_PRIVATE_KEY.
 *
 * Run with:
 *   VEIL_INTEGRATION=1 EDGE_PROVABLE_API_KEY=<key> \
