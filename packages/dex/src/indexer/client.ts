...
/** The dev indexer this client defaults to. */
export const DEFAULT_INDEXER_URL = 'https://amm-api.dev.provable.com'

/**
 * Options for {@link IndexerClient}.
 *
 * @property baseUrl Indexer origin. Defaults to the Provable dev indexer.
...
/** An indexer request that came back non-2xx, with the server's error body. */
export class IndexerError extends Error {
  constructor(
    ...
    super(`Indexer ${status} on ${path}: ${body}`)
    this.name = 'IndexerError'
  }
}

/**
 * Typed client for the off-chain AMM indexer REST API.
 *
 * The indexer is a trusted convenience layer — discovery, history, candles,
 * quotes, the faucet. Values that gate money movement (swap outputs,
 * blinded-address usage) MUST come from the chain reads instead. All
 * response types are generated from the service's own OpenAPI spec
 * (`pnpm regen-openapi`), so drift shows up as a type change, not a runtime
 * surprise.
 *
 ...
 * const route = await indexer.getRoute({ tokenIn, tokenOut, amountIn: 10n ** 18n })
 */
export class IndexerClient {
  ...
  constructor(options: IndexerClientOptions = {}) {
    this.baseUrl = (options.baseUrl ?? DEFAULT_INDEXER_URL).replace(/\/$/, '')
  ...
  private async request<T>(
    ...
    if (!res.ok) throw new IndexerError(res.status, path, await res.text())
   ...
   *
   * The indexer authenticates by signature: it issues a nonce message, the
  ...
  /** Registers a token with the indexer (auth-gated). */
  ...
  /** Reads a user's public/authorized balances (base units, as the indexer sees them). */
  ...
  /** Lists the on-chain operation schemas the indexer publishes. */
  ...
  /** Raw on-chain pool introspection (slot + tick statuses) via the indexer. */
