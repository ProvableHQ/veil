...
export async function main(argv: string[]): Promise<void> {
  ...
  switch (route.name) {
    case 'arc-to-aleo':
      ...
      await runArcToAleoExample({ amount: transferAmount(), execute })
      return
    case 'usdc-to-usdcx':
    ...
    case 'wbtc-to-aleo': {
      ...
      assign('EVM_PRIVATE_KEY', sourceKey)
      assign('ALEO_RECIPIENT', recipient)
      await runEthereumHyperlaneExample(asset, { amount: transferAmount(), execute })
      return
    }
    ...
    case 'sol-to-solana': {
      ...
      configureAleo(values, sourceKey)
      await runAleoHyperlaneExample(asset, { amount: transferAmount(), execute })
      return
    }
    case 'sol-to-aleo':
      ...
      assign('ALEO_RECIPIENT', recipient)
      await runSolanaHyperlaneExample({ amount: transferAmount(), execute })
  }
}
